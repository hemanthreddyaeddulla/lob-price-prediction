"""Inference-time copy of the feature engine. Kept self-contained (numpy only)
so the submission zip has no dependency on src/."""
...
class OnlineFeatureEngine:
    """O(1) feature engine for streaming inference. Must match src/features.py exactly."""
    ...
    def process(self, state: np.ndarray) -> np.ndarray:
        ...
        # crosses are scaled by 0.1 so they don't blow up the input magnitude
        offset += N_MICRO
